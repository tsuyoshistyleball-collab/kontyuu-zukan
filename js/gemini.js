...
const Gemini = (() => {
  ...
  const NAME_RULES = (one, egSp, egGrp, egNg) => [
    ...
    "   6つ そろえる ために、つぎの ものも 候補(こうほ)に 入(い)れて かまいません：",
    "   ・見(み)た目(め)が よく にた 別(べつ)の 種(しゅ)（同(おな)じ 科(か)の 仲間(なかま)など）",
    `   ・「${egGrp}」の ような 大(おお)きな まとまりの 名前(なまえ)`,
  ...
  const PROMPTS = {
    mushi: [
      ...
      "- riddle: この 虫(むし)の「なぞなぞ」。4さいが 声(こえ)に 出(だ)して 楽(たの)しめる 2〜3文(ぶん)。",
      "  その 虫(むし)に なりきって、じぶんの とくちょう（角(つの)・かま・はね・すむ ところ・鳴(な)き声(ごえ)）を",
      "  ヒントに して、さいごを「だあれ？」で しめて ください。",
      "  ※ なぞなぞの 中(なか)に 答(こた)えの 名前(なまえ)を 入(い)れては いけません。",
      "  れい:「ぼくの 頭(あたま)には 大(おお)きな 角(つの)が 1本(ぽん)。夏(なつ)の 夜(よる)、木(き)の みつを なめに いくよ。だあれ？」",
    ...
    hana: [
      ...
      "- riddle: この 花(はな)や 草(くさ)の「なぞなぞ」。4さいが 声(こえ)に 出(だ)して 楽(たの)しめる 2〜3文(ぶん)。",
      "  その 花(はな)や 草(くさ)に なりきって、じぶんの とくちょう（花(はな)びらの 形(かたち)・色(いろ)・さく きせつ・はえる ところ）を",
      "  ヒントに して、さいごを「だあれ？」で しめて ください。",
      "  ※ なぞなぞの 中(なか)に 答(こた)えの 名前(なまえ)を 入(い)れては いけません。",
      "  れい:「わたしの わたげは 風(かぜ)に のって 遠(とお)くまで 飛(と)ぶよ。春(はる)の 道(みち)ばたで 黄色(きいろ)く さくの。だあれ？」",
      "- riddle_hint: わからない ときの ヒント。1文(ぶん)で、名前(なまえ)は 入(い)れない。",
      "【かきかたの きまり】name と kana いがい の ぶんしょうは、かんじを つかい、",
      "  かんじの すぐ あとに よみがなを まるかっこで つけて ください。",
      "  れい：「大(おお)きな 角(つの)」「本州(ほんしゅう)の 林(はやし)」「6月(がつ)〜8月(がつ)」。",
    ...
    doubutsu: [
      ...
      "- riddle: この 動物(どうぶつ)の「なぞなぞ」。4さいが 声(こえ)に 出(だ)して 楽(たの)しめる 2〜3文(ぶん)。",
      "  その 動物(どうぶつ)に なりきって、じぶんの とくちょう（体(からだ)の 形(かたち)・耳(みみ)や しっぽ・鳴(な)き声(ごえ)・すむ ところ）を",
      "  ヒントに して、さいごを「だあれ？」で しめて ください。",
      "  ※ なぞなぞの 中(なか)に 答(こた)えの 名前(なまえ)を 入(い)れては いけません。",
      "  れい:「ぼくの 鼻(はな)は とっても 長(なが)くて、シャワーにも なるよ。耳(みみ)も 体(からだ)も 大(おお)きいんだ。だあれ？」",
      "- riddle_hint: わからない ときの ヒント。1文(ぶん)で、名前(なまえ)は 入(い)れない。",
      "【かきかたの きまり】name と kana いがい の ぶんしょうは、かんじを つかい、",
      "  かんじの すぐ あとに よみがなを まるかっこで つけて ください。",
  ...
  const DETAIL_PROMPT = {
    mushi: (name) => [
      ...
      "- riddle: この 虫(むし)の「なぞなぞ」。4さいが 楽(たの)しめる 2〜3文(ぶん)。",
      "  その 虫(むし)に なりきって、じぶんの とくちょうを ヒントに して、さいごを「だあれ？」で しめる。",
      "  ※ なぞなぞの 中(なか)に 答(こた)えの 名前(なまえ)を 入(い)れては いけません。",
      "  れい:「ぼくの 頭(あたま)には 大(おお)きな 角(つの)が 1本(ぽん)。夏(なつ)の 夜(よる)、木(き)の みつを なめに いくよ。だあれ？」",
      "- riddle_hint: わからない ときの ヒント。1文(ぶん)で、名前(なまえ)は 入(い)れない。",
    ...
    hana: (name) => [
      ...
      "- riddle: この 花(はな)や 草(くさ)の「なぞなぞ」。4さいが 楽(たの)しめる 2〜3文(ぶん)。",
      "  その 花(はな)や 草(くさ)に なりきって、じぶんの とくちょうを ヒントに して、さいごを「だあれ？」で しめる。",
      "  ※ なぞなぞの 中(なか)に 答(こた)えの 名前(なまえ)を 入(い)れては いけません。",
      "  れい:「わたしの わたげは 風(かぜ)に のって 遠(とお)くまで 飛(と)ぶよ。春(はる)の 道(みち)ばたで 黄色(きいろ)く さくの。だあれ？」",
      "- riddle_hint: わからない ときの ヒント。1文(ぶん)で、名前(なまえ)は 入(い)れない。",
      "【かきかたの きまり】ぶんしょうは、かんじを つかい、",
    ...
    doubutsu: (name) => [
      ...
      "- riddle: この 動物(どうぶつ)の「なぞなぞ」。4さいが 楽(たの)しめる 2〜3文(ぶん)。",
      "  その 動物(どうぶつ)に なりきって、じぶんの とくちょうを ヒントに して、さいごを「だあれ？」で しめる。",
      "  ※ なぞなぞの 中(なか)に 答(こた)えの 名前(なまえ)を 入(い)れては いけません。",
      "  れい:「ぼくの 鼻(はな)は とっても 長(なが)くて、シャワーにも なるよ。だあれ？」",
      "- riddle_hint: わからない ときの ヒント。1文(ぶん)で、名前(なまえ)は 入(い)れない。",
      "【かきかたの きまり】ぶんしょうは、かんじを つかい、",
  ...
  const RIDDLE_WORD = {
    mushi: ["虫(むし)", "ぼくの 頭(あたま)には 大(おお)きな 角(つの)が 1本(ぽん)。夏(なつ)の 夜(よる)、木(き)の みつを なめに いくよ。だあれ？"],
    hana: ["花(はな)や 草(くさ)", "わたしの わたげは 風(かぜ)に のって 遠(とお)くまで 飛(と)ぶよ。春(はる)の 道(みち)ばたで 黄色(きいろ)く さくの。だあれ？"],
    doubutsu: ["動物(どうぶつ)", "ぼくの 鼻(はな)は とっても 長(なが)くて、シャワーにも なるよ。だあれ？"],
  };
  async function makeRiddles(names, key, model, kind) {
    ...
    const prompt = [
      `つぎの ${one}の それぞれに ついて、4さいの こどもが 楽(たの)しめる`,
      "「なぞなぞ」を 1つずつ 作(つく)って ください。",
      `・その ${one}に なりきって、じぶんの とくちょうを ヒントに して ください。`,
      "・2〜3文(ぶん)。さいごは「だあれ？」で しめる。",
      "・※ なぞなぞの 中(なか)に 答(こた)えの 名前(なまえ)を 入(い)れては いけません。",
      `・れい:「${eg}」`,
      "・hint は わからない ときの ヒント。1文(ぶん)で、名前(なまえ)は 入(い)れない。",
      "・name は わたした 名前(なまえ)を そのまま かえして ください。",
      "【かきかたの きまり】かんじの すぐ あとに よみがなを まるかっこで つけて ください。",
      "  4さいが よめるように、たんごの あいだは はんかくスペースで くぎって ください。",
      "名前(なまえ)：",
      ...names.map((n) => "- " + n),
    ].join("\n");
